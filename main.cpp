...
enum class pageStatus {
	Free, 
	Divided, //if the block has size <= PAGE_SIZE / 2
	MultipageBlock // if the block has size > PAGE_SIZE / 2
};
...
void mem_free(void* addr) {
	...
	{
		for (uint8_t* block = page; block != page + PAGE_SIZE; block += headers[page].classSize) 
		{
			if (block + BLOCK_HEADER_SIZE == addr) 
			{
				*block = true;
			}
		}
...
void* mem_realloc(void* addr, size_t size) {
	...
	if (headers[page].status == pageStatus::Divided)
	{
		...
		if (pointer != nullptr)
		{
			...
			{
				*((uint8_t*)pointer + i) = *((uint8_t*)addr + i);
			}
			mem_free(addr);
		}
		return pointer;
